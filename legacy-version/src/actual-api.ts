...
export async function getTransactions(accountId: string, start: string, end: string): Promise<TransactionEntity[]> {
  // * Cache transactions by account and date range
  const cacheKey = `transactions:${accountId}:${start}:${end}`;
  return ensureConnection(() => cacheService.getOrFetch(cacheKey, () => api.getTransactions(accountId, start, end)));
}
...
export async function addTransactions(
  ...
  return ensureConnection(async () => {
    ...
    // * Invalidate specific transactions cache
    cacheService.invalidatePattern('transactions:*');
    // * Also invalidate accounts cache as balances might change
