...
## 2026-02-12 - [Weak CSP: Unsafe Inline Styles]
**Vulnerability:** The Content Security Policy (CSP) allowed `'unsafe-inline'` for styles (`style-src`). This weakened XSS protection by permitting arbitrary inline CSS injection, which could be used for data exfiltration or UI redressing.
**Learning:** Legacy or convenient inline styles in server-side rendered templates often block the adoption of strict CSP. Refactoring these to utility classes and using a nonce-based approach is necessary to close this gap without breaking functionality.
**Prevention:** I refactored the dashboard templates in `mcp-server/src/core/utils/dashboard.ts` to use CSS utility classes instead of inline `style` attributes. I updated `mcp-server/src/index.ts` to include these classes and attach a cryptographic `nonce` to the main `<style>` block, enabling a strict `style-src 'self' 'nonce-...'` policy in `security-headers.ts`.
