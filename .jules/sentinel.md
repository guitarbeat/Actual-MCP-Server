...
## 2024-05-26 - [Strict CORS Validation]
**Vulnerability:** The MCP server was configured with `Access-Control-Allow-Origin: *`, allowing any website to interact with the API. While intended for development convenience, this exposes local users to attacks from malicious websites if they run the server with `--sse` and without Bearer auth (or if auth is bypassed/leaked).
**Learning:** Development tools often default to permissive security for ease of use, but "ease of use" should not compromise safety by default. The assumption that "Poke MCP runs in browser" led to a wildcard policy, whereas a targeted whitelist (localhost + official inspector) serves the same purpose securely.
**Prevention:** I implemented a strict `corsMiddleware` in `src/core/transport/cors.ts` that whitelists `localhost`, `127.0.0.1`, and the official MCP Inspector. It also supports a `MCP_ALLOWED_ORIGINS` environment variable for custom deployments, ensuring the server is secure by default while remaining flexible.
