...
export async function getAccountHealthSummary(): Promise<AccountHealthItem[]> {
    ...
    const balances = await Promise.all(
        openAccounts.map((account) => getAccountBalance(account.id))
    );
 ...
 */
export async function getUpcomingSchedulesSummary(days: number = 14): Promise<UpcomingScheduleItem[]> {
    try {
...
export async function calculateTrends(month: string): Promise<TrendsSummary> {
    ...
    const [currentBudget, previousBudget] = await Promise.all([
        ...
        getBudgetMonth(prevMonth) as Promise<{ incomeAvailable?: number; toBudget?: number; categoryGroups?: Array<{ categories?: Array<{ spent?: number }> }> }>,
    ]);

    // Calculate total spending for each month
    const calculateSpending = (budget: { categoryGroups?: Array<{ categories?: Array<{ spent?: number }> }> }): number => {
        let total = 0;
