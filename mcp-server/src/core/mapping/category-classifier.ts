  ...
  classifyTransaction(
    ...
    if (transaction.amount > 0) {
      if (
        ...
        payeeLower.includes('employer')
      ) {
        const incomeCat = categories.find(
          (c) => c.is_income || c.name.toLowerCase().includes('income'),
        );
        if (incomeCat) return incomeCat.id;
      }
      if (payeeLower.includes('interest') || payeeLower.includes('dividend')) {
        const interestCat = categories.find(
          (c) =>
            c.name.toLowerCase().includes('interest') || c.name.toLowerCase().includes('dividend'),
        );
      ...
      for (const [payeeKeyword, categoryKeyword] of Object.entries(heuristics)) {
        if (payeeLower.includes(payeeKeyword)) {
          const matchedCategory = categories.find((c) =>
            c.name.toLowerCase().includes(categoryKeyword),
          );
      ...
      if (transaction.amount <= -100000 && transaction.amount % 10000 === 0) {
        // -$1000+ and even hundred
        const housingCat = categories.find(
          (c) => c.name.toLowerCase().includes('rent') || c.name.toLowerCase().includes('mortgage'),
        );
