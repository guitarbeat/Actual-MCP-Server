  ...
  inputSchema: zodToJsonSchema(
    z.object({
      ...
      dryRun: z
        ...
        .default(true)
        .describe(
          'If true, simulates the operation to ask for confirmation before modifying the budget.',
        ),
    }),
  ) as ToolInput,
};
...
export async function handler(args: unknown) {
  return executeToolAction(args, {
    ...
    execute: async (validated) => {
      ...
      const { activeBudgetId } = getConnectionState();
      if (!activeBudgetId) {
      ...
      await actualApi.internal.send('backup-load', {
        id: activeBudgetId,
        backupId: validated.backupId,
      });
    ...
    buildResponse: (_, result) => {
      if (result.dryRun) {
        return success(
          `Dry run: Ready to restore backup '${result.backupId}'. Call this tool again with dryRun=false to confirm and proceed.`,
        );
      }
