...
export async function handler(args: unknown) {
  return executeToolAction(args, {
    ...
    execute: async () => {
      ...
      const backups = (await actualApi.internal.send('backups-get', { id: activeBudgetId })) as {
        id: string;
        date: string;
      }[];
      if (!backups || backups.length === 0) {
        throw new Error(
          'Backup creation reported success, but could not list the newly created backup.',
        );
      }

      const latestBackup = backups.reduce(
        (latest: { id: string; date: string } | null, current) => {
